...
class SplashWindowController: NSWindowController {
    ...
    private func setupUI() {
        ...
        colorAnimation.values = [
            ...
            NSColor(calibratedWhite: 0.15, alpha: 1.0).cgColor,   // gris très foncé
            NSColor(calibratedWhite: 0.3, alpha: 1.0).cgColor,    // gris foncé
            NSColor(calibratedWhite: 0.6, alpha: 1.0).cgColor,    // gris moyen
            NSColor(calibratedWhite: 0.85, alpha: 1.0).cgColor,   // gris clair
            NSColor.white.cgColor                                 // blanc
        ]

        colorAnimation.keyTimes = [0, 0.2, 0.4, 0.6, 0.8, 1]

        // Durée totale (ajuste selon la durée de ton splash)
        colorAnimation.duration = 3.0

        // Laisser la dernière couleur affichée (blanc)
        colorAnimation.fillMode = .forwards
        colorAnimation.isRemovedOnCompletion = false

        // Appliquer l'animation
        contentView.layer?.add(colorAnimation, forKey: "backgroundFade")
